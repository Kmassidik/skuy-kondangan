<template>
  <div class="flex flex-col items-center justify-center">
    <transition v-if="showBox1" enter-active-class="animated fadeInUp" appear>
      <div class="my-5 w-5/6 flex flex-col items-center justify-center">
        <h1
          class="text-5xl font-bold great-vibes-regular tracking-wide text-center mt-8">
          Wedding Gift
        </h1>
        <div
          class="my-3 relative shadow-md flex flex-col items-center justify-center">
          <p
            class="text-justify font-normal text-base p-5 font-sans tracking-tighter"
            style="color: #722637">
            Doa restu anda merupakan karunia yang sangat berarti bagi kami. Jika
            memberi adalah ungkapan tanda kasih, anda dapat memberi kado secara
            cashless.
          </p>
          <button
            type="button"
            style="background-color: #9f0024"
            @click="showModal"
            class="font-bold p-2 my-3 text-white rounded tracking-wide flex items-center justify-center animate-bounce">
            <svg
              class="w-6 h-6 mr-3"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"></g>
              <g id="SVGRepo_iconCarrier">
                <path
                  d="M12 7V20M12 7H8.46429C7.94332 7 7.4437 6.78929 7.07533 6.41421C6.70695 6.03914 6.5 5.53043 6.5 5C6.5 4.46957 6.70695 3.96086 7.07533 3.58579C7.4437 3.21071 7.94332 3 8.46429 3C11.2143 3 12 7 12 7ZM12 7H15.5357C16.0567 7 16.5563 6.78929 16.9247 6.41421C17.293 6.03914 17.5 5.53043 17.5 5C17.5 4.46957 17.293 3.96086 16.9247 3.58579C16.5563 3.21071 16.0567 3 15.5357 3C12.7857 3 12 7 12 7ZM5 12H19V17.8C19 18.9201 19 19.4802 18.782 19.908C18.5903 20.2843 18.2843 20.5903 17.908 20.782C17.4802 21 16.9201 21 15.8 21H8.2C7.07989 21 6.51984 21 6.09202 20.782C5.71569 20.5903 5.40973 20.2843 5.21799 19.908C5 19.4802 5 18.9201 5 17.8V12ZM4.6 12H19.4C19.9601 12 20.2401 12 20.454 11.891C20.6422 11.7951 20.7951 11.6422 20.891 11.454C21 11.2401 21 10.9601 21 10.4V8.6C21 8.03995 21 7.75992 20.891 7.54601C20.7951 7.35785 20.6422 7.20487 20.454 7.10899C20.2401 7 19.9601 7 19.4 7H4.6C4.03995 7 3.75992 7 3.54601 7.10899C3.35785 7.20487 3.20487 7.35785 3.10899 7.54601C3 7.75992 3 8.03995 3 8.6V10.4C3 10.9601 3 11.2401 3.10899 11.454C3.20487 11.6422 3.35785 11.7951 3.54601 11.891C3.75992 12 4.03995 12 4.6 12Z"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"></path>
              </g>
            </svg>
            Kirim Hadiah
          </button>
        </div>
      </div>
    </transition>
    <Modal :visible="isModalVisible" @close="isModalVisible = false" />
  </div>
</template>

<script>
import Modal from "../modal/debitCard.vue";

export default {
  props: {
    isBankVisible: {
      type: Boolean,
      default: false,
    },
  },
  components: {
    Modal,
  },
  data() {
    return {
      show: this.isBankVisible,
      showBox1: false,
      isModalVisible: false,
    };
  },
  watch: {
    isBankVisible(newValue) {
      this.show = newValue;
      if (newValue) {
        this.triggerTransitions();
      }
    },
  },
  methods: {
    showModal() {
      this.isModalVisible = true;
    },
    triggerTransitions() {
      setTimeout(() => {
        this.showBox1 = true;
        this.$emit("visibleGallery");
      }, 500);
    },
  },
};
</script>

<style>
/* Add your component-specific styles here */
</style>
